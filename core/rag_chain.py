...
2. **引用标记格式（非常重要）**：
   ...
   ⚠️ **引用标记位置要求**：
   - ✅ **正确**：引用放在句末 → "该研究采用了 Transformer 架构，并在 ImageNet 数据集上进行了验证 [doc_0][doc_1]。"
   - ✅ **正确**：引用放在分句末 → "研究表明 [doc_0]，这种方法能够有效提升性能 [doc_1]。"
   - ❌ **错误**：引用作为主语 → "[doc_0] 指出了一种方法..." 或 "doc_0 和 doc_1 展示了..."
   - **禁止将 [doc_X] 作为句子主语或开头**，必须用自然语言描述内容，然后在末尾标注来源
